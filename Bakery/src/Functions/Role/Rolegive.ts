...
const slashCommandBuilder = new SlashCommandBuilder()
    .setName('역할지급')
    .setDescription('역할을 지급합니다')
    .addUserOption(option =>
        option
            .setName('유저')
            .setDescription('지급할 유저를 선택해주세요')
            .setRequired(true)
    )
    .addRoleOption(role =>
        role.setName('역할').setDescription('역할을 선택해주세요').setRequired(true)
    )
...
const Command = new SlashCommand(slashCommandBuilder, async (bot, interaction) => {
    ...
    await interaction.reply({content:interaction?.user.toString()+'님이 '+user1?.toString()+'님에게 '+role1?.toString()+'을 추가했습니다'});
